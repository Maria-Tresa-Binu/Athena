...
server.tool("toolkit_list_connectors", "List connectors available from the Toolkit backend.", {}, async () => json(await toolkit.connectors.list()));
server.tool("toolkit_authorize", "Start OAuth authorization for Gmail or Google Calendar. Open the returned redirectUrl in a browser.", {
...
server.tool("toolkit_wait_for_connection", "Wait for a previously started Toolkit OAuth authorization to finish.", { requestId: z.string() }, async ({ requestId }) => {
    const request = pendingAuthorizations.get(requestId);
    if (!request)
        return json({ error: "Unknown or expired authorization request" });
    try {
...
server.tool("toolkit_list_tools", "List available provider tools for a connected user. Use read='all' and write=[] for safe discovery.", {
...
server.tool("toolkit_search", "Search the Toolkit catalog for the best Gmail or Google Calendar tool for a request.", {
    query: z.string(),
    userId: z.string().default(defaultUserId),
    connectors: z.array(z.string()).default(["gmail", "google-calendar"]),
}, async ({ query, userId, connectors }) => json(await toolkit.tools.search(query, { userId, connectors })));
server.tool("toolkit_execute", "Execute one selected Toolkit tool. The bridge blocks write-capable actions unless TOOLKIT_ALLOW_WRITES=true.", {
    toolName: z.string(),
    userId: z.string().default(defaultUserId),
