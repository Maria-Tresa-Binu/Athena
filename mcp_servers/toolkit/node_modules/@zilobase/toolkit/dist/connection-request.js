...
export class ConnectionRequest {
    ...
    expiresAt;
    status;
    constructor(transport, snapshot) {
    ...
    async refresh(signal) {
        this.snapshot = await this.transport.request(`/v1/connection-requests/${encodeURIComponent(this.id)}`, { signal });
    ...
    async waitForConnection(options = {}) {
        ...
        const timeout = setTimeout(() => controller.abort(new Error("Connection request timed out")), timeoutMs);
        ...
        try {
            while (true) {
                ...
                if (snapshot.status === "failed") {
                    throw new ToolkitError(snapshot.failure?.message ?? "The connection request failed.", {
                ...
                await delay(Math.max(250, options.pollIntervalMs ?? 1_000), controller.signal);
            }
        }
        catch (error) {
            if (error instanceof ToolkitError)
                throw error;
            throw new ToolkitError(options.signal?.aborted
